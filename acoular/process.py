...
class SampleSplitter(InOut):
    ...
    **Key features**
    ...
    def register_object(self, *objects_to_register, buffer_size=None, buffer_overflow_treatment=None):
        """
        Registers one or more target objects to the :class:`SampleSplitter` object.
    ...
    def remove_object(self, *objects_to_remove):
        """
        Unregisters one or more objects from the :class:`SampleSplitter`.
    ...
    def result(self, num):
        """
        Yields data blocks from the buffer to the calling object.
...
class SamplesBuffer(InOut):
    ...
    def read_from_buffer(self, num):
        ...
        available samples. The index of the buffer is updated based on the :attr:`shift_index_by` setting.

...
The classes in this module are abstract base classes (ABCs) that provide a common interface for all
classes that generate an output via the generator :meth:`result` in block-wise manner. They are not
intended to be used directly, but to be subclassed by classes that implement the actual signal
processing.
...
class Generator(ABCHasStrictTraits):
    ...
    of the object, which is a hash of the object's attributes. This is used to check if the object's
...
class TimeOut(SamplesGenerator):
    """
    ABC for any signal processing block returning time domain signals.
...
class SpectraOut(SpectraGenerator):
    """
    ABC for any signal processing block returning frequency domain signals.
...
class InOut(SamplesGenerator, SpectraGenerator):
    """
    ABC for any signal processing block returning signals in the same domain as the input.
